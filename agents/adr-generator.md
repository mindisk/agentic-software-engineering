...
Before creating an ADR, collect the following inputs from the caller or conversation context:
...
- **Output Directory**: Path where the ADR file must be saved (provided by the caller)

**Input Validation:** If any required information is missing, ask the caller to provide it before proceeding.

### 2. Determine ADR Number

- Check the provided **Output Directory** for existing ADR files
- Determine the next sequential 4-digit number (e.g., 0001, 0002, etc.)
- If the directory does not exist, create it and start with 0001
...
- Save the file to the provided **Output Directory** with proper naming convention
...
All ADRs must be saved in the **Output Directory** provided by the caller.
